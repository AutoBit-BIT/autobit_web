...
export async function sendAccountApprovalEmail(
  email,
  name,
  subject = "Account Approved - Autobit"
) {
  const mailOptions = {
    from: process.env.EMAIL_USER,
    to: email,
...
export async function sendAccountRejectedEmail(
  email,
  name,
  subject = "Account Status - Autobit"
) {
  const mailOptions = {
    from: process.env.EMAIL_USER,
    to: email,
...
export async function sendQuestionApprovalEmail(
  ...
  name,
  questionTitle,
  subject = "Question Status - Autobit"
) {
  const mailOptions = {
    from: process.env.EMAIL_USER,
    to: email,
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p>Thank you for contributing to Autobit!</p>
        <p>Autobit Team</p>
      </div>
    `,
  };
